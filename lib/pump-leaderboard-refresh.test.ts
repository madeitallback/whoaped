...
describe("daily Pump leaderboard refresh", () => {
  ...
  it("keeps missing batch metrics null and calculates the transparent score", () => {
    const metrics = metricsFromDuneSummary({ address: wallet, swaps_30d: 12, last_activity: "2026-08-27T12:00:00Z", closed_positions: 10, win_rate: .7, capital_weighted_return: .4, realized_pnl_usd: 400, profit_factor: 2.5, median_winner_return: .8, median_loser_return: -.2 });
    expect(metrics).toMatchObject({ score: 69, winRate: .7, capitalWeightedReturn: .4, medianHoldSeconds: null, active: true, sampleConfidence: .5, profitFactor: 2.5, medianWinnerReturn: .8, medianLoserReturn: -.2 });
  });
  ...
  it("creates a stable daily profile without fabricated trades", () => {
    ...
    expect(profile).toMatchObject({ id: stablePumpProfileId(wallet), dataset: "pump_daily_v2", source: "pump", status: "partial", updatedAt: 123, trades: [] });
