...
export function rpcUrl() {
  return process.env.HELIUS_RPC_URL || (process.env.HELIUS_API_KEY
...
export async function rpcRetry<T>(operation: () => Promise<T>, attempts = 4): Promise<T> {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try { return await operation(); }
      ...
      await pause(500 * 2 ** attempt + Math.round(Math.random() * 150));
    }
  }
  throw lastError;
}
...
export function deriveBondingCurve(mint: PublicKey, tokenProgram = TOKEN_PROGRAM_ID) {
  ...
  return { bondingCurve, associatedBondingCurve: getAssociatedTokenAddressSync(mint, bondingCurve, true, tokenProgram) };
}

export function tokenProgramFor(owner: PublicKey) { return owner.equals(TOKEN_2022_PROGRAM_ID) ? TOKEN_2022_PROGRAM_ID : TOKEN_PROGRAM_ID; }
...
export function parseCurveAccount(data: Buffer) {
  ...
  const base = 8;
  return {
    realTokenReserves: read(base + 16),
    complete: data[base + 40] === 1,
...
export function parseMintInput(input: string) {
  ...
  try { return new PublicKey(pumpUrl?.[1] || value); } catch { return null; }
}
