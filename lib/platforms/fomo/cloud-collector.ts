...
export async function collectFomoLeaderboards(storageState: FomoStorageState) {
  ...
  try {
    ...
    await leaderboardButton.waitFor({ state: "visible", timeout: 45_000 }).catch(() => {
      throw new Error("The Fomo collector session expired and must be connected again.");
    });
    ...
    for (const item of WINDOWS) {
      ...
      const rows = await rowsLocator.evaluateAll((links) => links.flatMap((link, index) => {
        ...
        const realizedPnlUsd = moneyText
          ? Number(moneyText.replace("$", "").replaceAll(",", ""))
          : null;
        ...
        return [{
          ...
          volumeUsd: null,
          tradeCount: compactCount ? Number(compactCount.slice(0, -1)) : null,
          followerCount: null,
        }];
      }));
