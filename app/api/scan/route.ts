...
import { advanceHeliusJobs, enqueueHeliusCurveIndex, heliusIndexState } from "@/lib/helius-index";
...
async function scan(mint: PublicKey): Promise<ScanResponse> {
  ...
  const heliusState = await heliusIndexState(mint.toBase58()).catch(() => "not_started");
  ...
  if (heliusState === "queued" || heliusState === "running") warnings.push("Full Pump.fun curve history is indexing in safe background pages; current curve data is a lower bound.");
