import { enqueueBuyerHistory, enqueueFomoTokenCapture, enqueueHolderSnapshot, enqueueTokenRefresh, persistTokenScan } from "@/lib/data/repository";
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      fomoCaptureQueued = await enqueueFomoTokenCapture(scan.mint);
      historyQueued = await enqueueBuyerHistory(scan);
      if (queued || holderSnapshotQueued || fomoCaptureQueued || historyQueued.curve || historyQueued.pumpswap) {
    ...
    return Response.json({ ...scan, persistence: { persisted, queued, holderSnapshotQueued, fomoCaptureQueued, historyQueued } });
  } catch (error) {
