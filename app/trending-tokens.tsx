...
export function TrendingTokens() {
  ...
  return <section className="panel trending-board" aria-live="polite" aria-busy={loading}>
    <div className="board-head"><div><p className="eyebrow">LIVE DISCOVERY / DEXSCREENER SOLANA</p><h2>Trending now</h2><p>Open a token to see verified Pump buyers, current holders, Fomo identities, and attributable thesis evidence.</p></div><button className="row-open" type="button" onClick={() => void load()} disabled={loading}>{loading ? "REFRESHING…" : "REFRESH ↻"}</button></div>
    ...
    {!loading && !error && tokens.length === 0 ? <p className="inline-notice">No Solana discovery tokens are available right now.</p> : null}
  </section>;
}
