...
export function SocialLeaderboard({ preview = false }: { preview?: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/leaderboard/social", { signal: controller.signal }).then(async (response) => {
  ...
  return <section className="terminal-panel social-board" aria-live="polite" aria-busy={loading}>
    ...
    {!loading && payload.sources?.pump === "awaiting_analysis" ? <p className="source-warning">Pump rankings appear as verified wallet analyses are indexed; no placeholder traders are invented.</p> : null}
