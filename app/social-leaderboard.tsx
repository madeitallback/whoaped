...
export function SocialLeaderboard({ preview = false }: { preview?: boolean }) {
  ...
  return <section className="terminal-panel social-board" aria-live="polite" aria-busy={loading}>
    ...
    <div className="social-table-wrap"><table className="social-table"><thead><tr><th>RANK</th><th>TRADER</th><th>PRIMARY SIGNAL</th><th>FOLLOWERS</th><th>WIN RATE</th><th>WEIGHTED RETURN</th><th>MEDIAN HOLD</th><th>ACTIVITY / SAMPLE</th><th /></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><span className="platform-rank">{row.platform === "fomo" ? "F" : "P"}{String(row.platformRank).padStart(2, "0")}</span></td><td><div className="trader-cell">{row.avatarUrl ? <Image src={row.avatarUrl} alt="" width={32} height={32} sizes="32px" /> : <span className={`avatar-fallback ${row.platform}`}>{row.label.slice(0, 1).toUpperCase()}</span>}<div><b>{row.label}</b><span>@{row.handle}</span></div><em className={`platform-tag ${row.platform}`}>{row.platform}</em></div></td><td><strong className={(row.primaryMetric ?? 0) >= 0 ? "up" : "down"}>{row.platform === "fomo" ? usd(row.primaryMetric) : row.primaryMetric ?? "—"}</strong><small>{row.metricLabel}</small></td><td>{row.followers === null ? "—" : row.followers.toLocaleString()}</td><td>{pct(row.winRate)}</td><td className={(row.weightedReturn ?? 0) >= 0 ? "up" : "down"}>{pct(row.weightedReturn)}</td><td>{formatDuration(row.medianHoldSeconds)}</td><td><b>{row.platform === "fomo" ? `${row.trades24h ?? 0} trades · ${usd(row.volume24hUsd)}` : row.lastActivityAt ? `${new Date(row.lastActivityAt * 1000).toLocaleDateString()}${row.profitFactor !== null ? ` · PF ${row.profitFactor.toFixed(2)}` : ""}` : "Not indexed"}</b><small>{row.sampleLabel}</small></td><td><a href={row.profileUrl} target="_blank" rel="noreferrer">OPEN ↗</a></td></tr>)}{!loading && rows.length === 0 ? <tr><td colSpan={9} className="empty-cell">No ranked traders for this source yet.</td></tr> : null}</tbody></table></div>
