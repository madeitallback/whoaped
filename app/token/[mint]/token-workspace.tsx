...
export function TokenWorkspace({ mint }: { mint: string }) {
  ...
  const socialActors = useMemo(() => {
    ...
    return [...rows.values()].sort((a, b) => {
      const aPosition = activityByWallet.get(a.wallet)?.status || "not_held";
      const bPosition = activityByWallet.get(b.wallet)?.status || "not_held";
  ...
  return <main className="token-page">
    ...
    {scan && <>
      <section className="panel token-header"><div className="panel-head"><div><p className="eyebrow">TOKEN SOCIAL INTELLIGENCE / {scan.token.isPumpFun ? "PUMP.FUN" : "SOLANA"}</p><h1>{scan.token.name} <small>${scan.token.symbol}</small></h1></div><span className="badge">{scan.coverage.buyerState.toUpperCase()}</span></div><p className="token-ca">{scan.mint}</p><p className="token-readout"><b>{socialActors.length} verified social actor{socialActors.length === 1 ? "" : "s"}</b> across Pump and Fomo. <b>{positionSummary.current} still hold or trimmed</b>, {positionSummary.exited} verified exits, and {theses.length} attributable public {theses.length === 1 ? "thesis" : "theses"}.</p><div className="token-status"><span>{scan.token.graduated ? "GRADUATED" : scan.token.graduated === false ? "ON CURVE" : "NON-PUMP"}</span><span>{scan.coverage.scannedSignatures} / {scan.coverage.signatureLimit} recent signatures</span><span>{enriching ? "Resolving Pump + Fomo identities…" : `${Object.keys(pumpProfiles).length} Pump / ${Object.keys(identities).length} Fomo verified`}</span></div></section>
      <section className="token-metrics"><TokenMetric label="SOCIAL ACTORS" value={enriching ? "…" : number(socialActors.length)} detail="verified Pump + Fomo identities" /><TokenMetric label="STILL IN" value={number(positionSummary.current)} detail="holding or trimmed positions" /><TokenMetric label="VERIFIED EXITS" value={number(positionSummary.exited)} detail={`${positionSummary.unproven} exits remain unproven`} /><TokenMetric label="PUBLIC THESES" value={number(theses.length)} detail={thesisProvider === "refreshed" ? "live provider refresh" : thesisProvider} /></section>
      <p className="coverage-line">ⓘ {scan.coverage.note}</p>
      <section className="panel token-table" id="social-actors"><div className="panel-head"><div><p className="eyebrow">PUMP + FOMO / VERIFIED IDENTITIES</p><h2>Who aped—and are they still in?</h2></div><span className="badge">{socialActors.length} VERIFIED</span></div><div className="table-wrap"><table><thead><tr><th>#</th><th>ACTOR</th><th>SOURCE</th><th>POSITION</th><th>WALLET</th><th>ROLE</th><th>CONFIDENCE</th><th /></tr></thead><tbody>{socialActors.map((actor, index) => { const position = activityByWallet.get(actor.wallet); return <tr key={`${actor.platform}:${actor.profileId}:${actor.wallet}`}><td>{index + 1}</td><td><a href={actor.profileUrl} target="_blank" rel="noreferrer">{actor.handle ? `@${actor.handle}` : actor.displayName || actor.platformProfileId}</a></td><td><span className={`source-pill ${actor.platform}`}>{actor.platform}</span></td><td><span className={`position-state ${position?.status || "not_held"}`}>{(position?.status || "indexing").replace("_", " ").toUpperCase()}</span></td><td>{short(actor.wallet)}</td><td>{actor.relationship.toUpperCase()}</td><td>{actor.confidence.toUpperCase()}</td><td><Link href={`/?wallet=${encodeURIComponent(actor.wallet)}&label=${encodeURIComponent(actor.handle || short(actor.wallet))}`}>ANALYZE ↗</Link></td></tr>; })}{socialActors.length === 0 && <tr className="table-empty"><td colSpan={8}><b>No verified social identity in the measured cohort yet.</b><span>Wallet activity remains visible below. WHOAPED never guesses an identity from a display name.</span></td></tr>}</tbody></table></div></section>
