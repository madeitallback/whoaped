...
export function TokenWorkspace({ mint }: { mint: string }) {
  ...
  return <main className="token-page">
    ...
    {scan && <>
      ...
      <TokenSocialCockpit actors={socialActors} theses={theses} positions={activityByWallet} enriching={enriching} />
...
function TokenSocialCockpit({ actors, theses, positions, enriching }: { actors: TokenSocialActor[]; theses: TokenThesisEvidence[]; positions: Map<string, TokenWalletActivity>; enriching: boolean }) {
  ...
  return <section className="token-cockpit">
    <div className="terminal-panel actor-matrix"><header className="section-head"><div><span className="kicker">CROSS-PLATFORM HOLDER MAP</span><h2>Who aped—and who still holds?</h2><p>Verified identities only. Position state comes from reconciled on-chain activity.</p></div><div className="source-health"><span><i className="dot pump" />{counts.pump} PUMP</span><span><i className="dot fomo" />{counts.fomo} FOMO</span></div></header><div className="terminal-tabs cockpit-tabs">{(["all", "pump", "fomo"] as const).map((item) => <button type="button" className={platform === item ? "active" : ""} onClick={() => setPlatform(item)} key={item}>{item === "all" ? `ALL ${actors.length}` : `${item.toUpperCase()} ${counts[item]}`}</button>)}</div><div className="actor-list">{visible.map((actor) => { const position = positions.get(actor.wallet); const status = position?.status || (actor.relationship === "holder" || actor.relationship === "both" ? "holding" : "not_held"); const actorTheses = thesesByProfile.get(actor.profileId) || []; return <article className="actor-card" key={`${actor.platform}:${actor.profileId}:${actor.wallet}`}><span className={`avatar-fallback ${actor.platform}`}>{(actor.handle || actor.displayName || "?").slice(0, 1).toUpperCase()}</span><div className="actor-identity"><a href={actor.profileUrl} target="_blank" rel="noreferrer">{actor.handle ? `@${actor.handle}` : actor.displayName || actor.platformProfileId}</a><span>{short(actor.wallet)} · {actor.relationship.replace("_", " ")}</span></div><em className={`platform-tag ${actor.platform}`}>{actor.platform}</em><span className={`position-state ${status}`}>{status.replace("_", " ").toUpperCase()}</span><div className="actor-proof"><b>{position ? `${position.buyTxCount} buys / ${position.sellTxCount} sells` : "Position indexing"}</b><small>{actorTheses.length ? `${actorTheses.length} public thesis source${actorTheses.length === 1 ? "" : "s"}` : "No attributable thesis"}</small></div><a className="open-arrow" href={actor.profileUrl} target="_blank" rel="noreferrer">↗</a></article>; })}{!enriching && visible.length === 0 ? <div className="cockpit-empty"><b>No verified {platform === "all" ? "social" : platform} identity in this measured cohort.</b><p>Wallet activity remains available below; WHOAPED never guesses who owns a wallet.</p></div> : null}{enriching ? <div className="cockpit-empty"><b>Resolving Pump + Fomo identities…</b></div> : null}</div></div>
    <aside className="terminal-panel thesis-feed"><header className="section-head"><div><span className="kicker">LIVE THESIS FEED</span><h2>Why they aped</h2></div><span className="count-badge">{theses.length}</span></header><div className="thesis-stream">{theses.slice(0, 12).map((thesis) => <article key={thesis.id}><div><a href={thesis.profileUrl} target="_blank" rel="noreferrer">{thesis.handle ? `@${thesis.handle}` : thesis.displayName || thesis.platformProfileId}</a><em className={`platform-tag ${thesis.platform}`}>{thesis.platform}</em></div><p>{thesis.sourceText}</p><footer><time>{new Date(thesis.publishedAt || thesis.capturedAt).toLocaleString()}</time><a href={thesis.sourceUrl} target="_blank" rel="noreferrer">SOURCE ↗</a></footer></article>)}{theses.length === 0 ? <div className="cockpit-empty"><b>No public thesis found.</b><p>We leave this empty instead of inferring intent from a buy.</p></div> : null}</div></aside>
  </section>;
}
