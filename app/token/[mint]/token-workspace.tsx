...
export function TokenWorkspace({ mint }: { mint: string }) {
      ...
      try {
        ...
        const loadPersistedActors = async () => {
          ...
          if (thesisResponse.ok) {
            const thesisBody = await thesisResponse.json() as { evidence?: TokenThesisEvidence[] };
  ...
  return <main className="token-page">
    ...
    {scan && <>
      ...
      <section className="panel thesis-panel"><div className="panel-head"><div><p className="eyebrow">ATTRIBUTABLE PUBLIC EVIDENCE</p><h2>Why they aped</h2></div><span className="badge">{theses.length} SOURCES</span></div><div className="thesis-list">{theses.map((thesis) => <article key={thesis.id}><div><a href={thesis.profileUrl} target="_blank" rel="noreferrer">{thesis.handle ? `@${thesis.handle}` : thesis.displayName || thesis.platformProfileId}</a><span className={`source-pill ${thesis.platform}`}>{thesis.platform}</span><span>{thesis.relevance.toUpperCase()}</span></div><p>{thesis.sourceText}</p><footer><a href={thesis.sourceUrl} target="_blank" rel="noreferrer">OPEN ORIGINAL ↗</a><span>Published {thesis.publishedAt ? new Date(thesis.publishedAt).toLocaleString() : "time unavailable"} · captured {new Date(thesis.capturedAt).toLocaleString()}</span></footer></article>)}{theses.length === 0 && <div className="thesis-empty"><b>No public thesis captured.</b><span>WHOAPED will never infer a thesis from a wallet transaction. Evidence appears only with an attributable Pump/Fomo source.</span></div>}</div></section>
