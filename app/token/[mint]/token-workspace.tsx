...
import { mergeTokenHolders } from "@/lib/token-intel/unified-holders";
...
export function TokenWorkspace({ mint }: { mint: string }) {
  ...
  const cohortRows = useMemo(() => {
    ...
    for (const holder of scan?.holders || []) {
      const current = rows.get(holder.owner);
      rows.set(holder.owner, {
        wallet: holder.owner,
        balanceUi: holder.uiAmount,
        pctOfSupply: holder.pctOfSupply,
        firstBuyAt: current?.firstBuyAt ?? null,
        lastBuyAt: current?.lastBuyAt ?? null,
        lastSellAt: current?.lastSellAt ?? null,
        buyTxCount: current?.buyTxCount ?? 0,
        sellTxCount: current?.sellTxCount ?? 0,
        status: current?.status === "trimmed" ? "trimmed" : "holding",
        observedAt: scan?.updatedAt || current?.observedAt || new Date(0).toISOString(),
      });
    }
    ...
    return [...rows.values()].sort((left, right) => right.balanceUi - left.balanceUi || right.buyTxCount - left.buyTxCount).slice(0, 2000);
  ...
  const holderRows = useMemo(() => mergeTokenHolders(cohortRows, socialActors), [cohortRows, socialActors]);
  const socialHolderCount = useMemo(() => holderRows.filter((row) => row.profiles.length > 0).length, [holderRows]);
  ...
  return <main className="token-page">
    ...
    {scan && <>
      ...
      <TokenSocialCockpit holders={holderRows} theses={theses} performance={socialPerformance} priceUsd={scan.token.priceUsd} symbol={scan.token.symbol} enriching={enriching} />
...
function TokenSocialCockpit({ holders, theses, performance, priceUsd, symbol, enriching }: { holders: ReturnType<typeof mergeTokenHolders>; theses: TokenThesisEvidence[]; performance: SocialBoardRow[]; priceUsd: number | null; symbol: string; enriching: boolean }) {
  const [platform, setPlatform] = useState<"all" | "pump" | "fomo">("all");
  const [thesisPlatform, setThesisPlatform] = useState<"all" | "pump" | "fomo">("all");
  const visible = holders.filter((holder) => platform === "all" || holder.profiles.some((profile) => profile.platform === platform));
  ...
  const counts = { pump: holders.filter((holder) => holder.profiles.some((profile) => profile.platform === "pump")).length, fomo: holders.filter((holder) => holder.profiles.some((profile) => profile.platform === "fomo")).length };
  ...
  return <section className="token-cockpit">
    <div className="terminal-panel actor-matrix"><header className="section-head"><div><span className="kicker">PUMP + FOMO HOLDER MAP</span><h2>Every holder, profile, wallet, and amount</h2><p>One on-chain wallet per row. Pump and Fomo profiles are attached to that position without duplicating it.</p></div><div className="source-health"><span><i className="dot pump" />{counts.pump} PUMP</span><span><i className="dot fomo" />{counts.fomo} FOMO</span></div></header><div className="terminal-tabs cockpit-tabs">{(["all", "pump", "fomo"] as const).map((item) => <button type="button" className={platform === item ? "active" : ""} onClick={() => setPlatform(item)} key={item}>{item === "all" ? `ALL HOLDERS ${holders.length}` : `${item.toUpperCase()} PROFILES ${counts[item]}`}</button>)}</div><div className="actor-list">{visible.map((holder) => { const profileTheses = holder.profiles.flatMap((profile) => thesesByProfile.get(profile.profileId) || []); return <article className="holder-card" key={holder.wallet}><div className="holder-rank">#{holders.indexOf(holder) + 1}</div><div className="holder-profiles">{holder.profiles.length ? holder.profiles.map((profile) => { const ranked = profile.platform === "pump" ? performanceByWallet.get(holder.wallet.toLowerCase()) : profile.handle ? performanceByHandle.get(profile.handle.toLowerCase()) : undefined; return <a className={`holder-profile ${profile.platform}`} href={profile.profileUrl} target="_blank" rel="noreferrer" key={`${profile.platform}:${profile.platformProfileId}`}><span className={`avatar-fallback ${profile.platform}`}>{(profile.handle || profile.displayName || "?").slice(0, 1).toUpperCase()}</span><span><b>{profile.handle ? `@${profile.handle}` : profile.displayName || profile.platformProfileId}</b><small>{profile.platform.toUpperCase()}{ranked ? ` · ${profile.platform === "fomo" ? `${usd(ranked.pnl24hUsd)} 24H` : `${pct(ranked.winRate)} WIN`}` : ""}</small></span></a>; }) : <span className="unidentified-holder"><b>Unidentified holder</b><small>No verified Pump/Fomo profile yet</small></span>}</div><a className="holder-wallet" href={`https://solscan.io/account/${encodeURIComponent(holder.wallet)}`} target="_blank" rel="noreferrer"><b>{short(holder.wallet)}</b><small>OPEN WALLET ↗</small></a><div className="holder-balance"><b>{number(holder.position.balanceUi)} ${symbol}</b><small>{usd(priceUsd === null ? null : holder.position.balanceUi * priceUsd)} · {percent(holder.position.pctOfSupply)} supply</small></div><span className={`position-state ${holder.position.status}`}>{holder.position.status.replace("_", " ").toUpperCase()}</span><div className="holder-evidence"><b>{holder.position.buyTxCount || holder.position.sellTxCount ? `${holder.position.buyTxCount} buys / ${holder.position.sellTxCount} sells` : "Current balance verified"}</b><small>{profileTheses.length ? `${profileTheses.length} attributable thesis source${profileTheses.length === 1 ? "" : "s"}` : "No attributable thesis captured"}</small></div></article>; })}{!enriching && visible.length === 0 ? <div className="cockpit-empty"><b>No {platform === "all" ? "current holder" : `${platform} profile linked to a current holder`} yet.</b><p>WHOAPED keeps the on-chain holder visible even when no social identity can be verified.</p></div> : null}{enriching ? <div className="cockpit-empty"><b>Resolving Pump + Fomo identities…</b></div> : null}</div></div>
