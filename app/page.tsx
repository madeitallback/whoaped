...
function FollowerEdgeCard({ metrics, label, loading, error }: { metrics: FollowerEdgeMetrics | null; label: string; loading: boolean; error: string }) { if (!loading && !metrics && !error) return null; return <section className="panel follower-edge-card" id="pump-follower-edge" aria-live="polite" aria-busy={loading}><div className="panel-head"><div><p className="eyebrow">PUMP / SOCIAL SIGNAL</p><h2>{label ? `@${label}` : "Follower Edge"}</h2></div><span className="badge">ACTIVE TRADERS / 30D</span></div>{loading && <div className="edge-loading"><b>CALCULATING FOLLOWER EDGE…</b><p>Sampling several depths of the public follower list, then checking realized performance.</p></div>}{error && <p className="error">{error}</p>}{metrics && <><div className="edge-primary"><strong>{metrics.followerEdge === null ? "—" : percent(metrics.followerEdge)}</strong><div><span>PROFITABLE ACTIVE FOLLOWERS</span><p>{metrics.profitableFollowers} profitable / {metrics.scorableFollowers} scorable followers</p></div></div><div className="edge-grid"><Metric label="ACTIVE 30D" value={`${metrics.activeFollowers30d} / ${metrics.sampledFollowers}`} /><Metric label="SCORABLE" value={`${metrics.scorableFollowers}`} /><Metric label="MEDIAN WIN RATE" value={percent(metrics.medianFollowerWinRate)} /><Metric label="MEDIAN RETURN" value={percent(metrics.medianFollowerReturn)} tone={(metrics.medianFollowerReturn ?? 0) >= 0 ? "positive" : "negative"} /><Metric label="PUBLIC SAMPLE" value={`${metrics.sampledFollowers} / ${metrics.visibleFollowers.toLocaleString()}`} /><Metric label="CONFIDENCE" value={metrics.confidence.toUpperCase()} /></div><p className="edge-method">Rank-stratified public Pump sample · {metrics.accessibleFollowers.toLocaleString()} accessible relations · {metrics.metricVersion}</p>{metrics.notices.map((notice) => <p className="inline-notice" key={notice}>{notice}</p>)}</>}</section>; }
