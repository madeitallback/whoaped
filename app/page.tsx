...
export default function Home() {
  ...
  const rows = useMemo(() => {
    ...
    return data.pumpfunBuyers.wallets.filter(x => filter === "all" || x.bucket === filter || (filter === "holding" && x.stillHolds));
  }, [data, tab, filter]);
  ...
  return <main>
    ...
    {data && <>
      ...
      <section className="section-head"><div><p className="eyebrow">ADDRESS INTELLIGENCE</p><h2>Who is still holding?</h2></div><div className="tabs"><button className={tab === "holders" ? "active" : ""} onClick={() => { setTab("holders"); setFilter("all"); }}>Holders <small>{data.holders.length}</small></button><button className={tab === "buyers" ? "active" : ""} onClick={() => { setTab("buyers"); setFilter("all"); }}>Buyers <small>{data.pumpfunBuyers.uniqueBuyers}</small></button></div></section>
      <section className="table-card"><div className="filters">{(tab === "holders" ? [["all", "All"], ["fomo", "FOMO"], ["pumpfun_curve", "Curve"], ["creator", "Creator"], ["other", "Other"]] : [["all", "All"], ["fomo", "FOMO"], ["pumpfun", "Pump.fun"], ["holding", "Still holding"]]).map(([key, label]) => <button key={key} className={filter === key ? "selected" : ""} onClick={() => setFilter(key)}>{label}</button>)}</div>
        <div className="table-wrap"><table><thead><tr><th>#</th><th>Label</th><th>Owner</th><th>Handle</th><th>{tab === "holders" ? "Tokens" : "Buy txs"}</th><th>% supply</th><th>Links</th></tr></thead><tbody>{rows.map((row, index) => <Row key={row.owner} row={row} index={index} buyers={tab === "buyers"} copy={copy} />)}</tbody></table></div>
