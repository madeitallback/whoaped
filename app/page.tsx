...
function SupplyChart({ points }: { points: SupplySnapshot[] }) {
  ...
  const priceLine = points.map((point, index) => { if (point.priceUsd === null) return null; const x = inset + (width - inset * 2) * (points.length === 1 ? .5 : index / (points.length - 1)); const y = height - inset - ((priceMax === priceMin ? .5 : (point.priceUsd - priceMin) / (priceMax - priceMin)) * (height - inset * 2)); return `${x},${y}`; }).filter((value): value is string => value !== null).join(" ");
  ...
  return <section className="table-card" style={{ padding: 20, marginTop: 18 }}><div className="section-head" style={{ marginBottom: 10 }}><div><p className="eyebrow">OWNERSHIP HISTORY</p><h2>{showPrice ? "Price alongside supply" : "Supply held over time"}</h2></div><div><button className={showPrice ? "selected" : ""} onClick={() => setShowPrice(value => !value)} disabled={!prices.length}>Price overlay</button><small className="muted"> {latest ? `${number(latest.holderCount)} holders · ${latest.holderIndexComplete ? "complete holder index" : "partial index"}` : "Snapshots start after the first scan"}</small></div></div>{points.length ? <><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Supply share history" style={{ width: "100%", height: 220, display: "block" }}><line x1={inset} x2={width - inset} y1={height - inset} y2={height - inset} stroke="rgba(255,255,255,.15)" /><polyline points={line("fomoPctOfSupply")} fill="none" stroke="#ff4ea3" strokeWidth="3" /><polyline points={line("preGradPctOfSupply")} fill="none" stroke="#ffc83d" strokeWidth="3" /><polyline points={line("postGradPctOfSupply")} fill="none" stroke="#67a8ff" strokeWidth="3" />{showPrice && priceLine && <polyline points={priceLine} fill="none" stroke="#7dffbd" strokeWidth="2" strokeDasharray="5 4" />}</svg><div className="filters" style={{ padding: 0 }}><span className="handle">● FOMO {pct(latest?.fomoPctOfSupply || 0)}</span><span style={{ color: "#ffc83d" }}>● Pre-grad {pct(latest?.preGradPctOfSupply || 0)}</span><span style={{ color: "#67a8ff" }}>● Post-grad {pct(latest?.postGradPctOfSupply || 0)}</span>{showPrice && <span style={{ color: "#7dffbd" }}>● Price {usd(latest?.priceUsd || null)} · liq {usd(latest?.liquidityUsd || null)}</span>}<span className="muted">{time(points[0].observedAt)} → {time(latest!.observedAt)}</span></div></> : <p className="empty">No ownership snapshots yet. The chart begins automatically as this token is indexed.</p>}</section>;
}
