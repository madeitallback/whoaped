...
Production verification: functional commit `320e263e8249579122712801d56c5fb6cb923733` deployed as `dpl_LnSMFESXDDMriyKGT9p15xn3XFgd` (`READY`) at `https://whoaped-phi.vercel.app`. Health returned RPC reachable, Supabase ready, and all four server providers configured. The canonical mixed board returned 100 live Fomo rows and 23 Pump rows. Its forced v2 refresh completed one bounded Dune batch and persisted all 20 automatic Pump profiles with sample confidence and profit factor; Pump changed from `verified_analysis_refreshing` to `daily_live`. The previous 100-score two-position outlier is now a 66 score with 10% confidence. Visual production checks passed on `/leaderboard` and the real `aura` token cockpit with no browser console errors; Vercel reported no runtime errors in the post-deploy window.
...
- [ ] Finish the durable authorized collector. The encrypted one-time dedicated-account bootstrap, five-minute Supabase Cron, lease/backoff state, four-window persistence, tests, and Vercel Chromium packaging are implemented. Production verification on 2026-08-27 proved the interactive login succeeds, but Privy's client-managed session does not survive restoration in a fresh serverless Chromium instance; after resource optimization, the remaining production error is `The Fomo collector session expired and must be connected again.` Move the collector to a persistent cloud browser/profile (self-hosted container with a durable volume or a managed persistent-browser context), then require two successful scheduled refreshes before checking this item. Never claim H24 operation from a successful bootstrap alone.
- [x] Implement the managed persistent-browser path with Browserbase Contexts, CDP collection, explicit session release/persistence, a one-time Live View login command, server-only environment validation, Vercel fallback selection, and the production runbook in `docs/fomo-browserbase-production.md`.
- [x] Provision the user's Browserbase project/API key, create the persistent Context through `pnpm fomo:browserbase:connect`, add the three encrypted Production variables to Vercel, redeploy, and record two consecutive successful scheduled refreshes with non-zero counts.
- [ ] Add launch-grade rate limiting, provider budgets, monitoring, and alerting.

Browserbase production completion — 2026-08-28:

- The dedicated Fomo account was authenticated once in a Browserbase persistent Context; the API key remains server-only in ignored local env and encrypted Vercel Production variables.
...
- [x] Verify the first deployed token-capture job and reject its initial false-positive result: the target token was absent from Fomo, while the app's global leaderboard remained mounted. The 100 non-mapped observations were removed rather than shown as fake Fomo holders.
- [x] Harden token capture: explicitly detect Fomo's `Token not found` state, require the token's `Holders` surface, ignore the left leaderboard/right account controls, and only accept a visible token-unit amount from a holder's own row. No dollar PnL, rank, follower count, or screenshot-like value can create a wallet link.
- [x] Keep the social board unified: Fomo remains one row type in the same combined board. When a Fomo profile has a chain-proven wallet, WHOAPED schedules the identical 90-day Dune wallet metric pass used for Pump and fills win rate/weighted return from that verified wallet. Metrics remain null, never fabricated, before proof.
- [x] Validate the working FomoScan key against its documented `/v2/me`, trader leaderboard, trending-token, thesis-by-token, and identity-by-handle endpoints. Use the verified `/v2/user/wallet/{address}` endpoint as the live resolver for every on-chain holder missing a stored Fomo identity; persist verified profile/wallet links and avatar URLs for reloads.
- [x] Make FomoScan active whenever its server-only key is present (unless explicitly disabled with `FOMOSCAN_FALLBACK_ENABLED=false`), rather than requiring an easy-to-forget secondary flag. It supplies profile attribution and live theses; it does not fabricate a per-token Fomo holder roster that the API does not expose.
- [x] Promote FomoScan's documented 24h trader board to the production leaderboard source, with a shared Vercel data-cache refresh every five minutes (8,640 provider reads per 30-day month at maximum steady state, not one request per visitor). The prior authorized first-party snapshot is now the outage fallback; the last successful provider result remains available for 15 minutes if both sources are transiently unavailable.
- [x] Enrich every FomoScan leaderboard row from already verified WHOAPED wallet links and cached Dune wallet metrics. Fomo's own 24h PnL always renders from FomoScan; win rate, weighted return, and median hold render only after the Fomo handle has a verified wallet link, never inferred from a name.
- [x] Normalize the daily board depth to a 50-profile cohort per source. Pump's public directory exposes 50 profiles and a single daily Dune batch supports all 50; FomoScan's 100-row response is bounded to the same 50 rows for an understandable combined view.
- [x] Treat a prior fresh 20-wallet Pump snapshot as incomplete after the cohort expansion, so the next social-board read safely starts the single new 50-wallet batch instead of waiting for tomorrow's cron.
- [x] Replace the opaque Pump score in the board with realized PnL, explicitly labelled `FOMO / rolling 24h` or `WHOAPED / 90d closed`. The daily Pump pass enriches public Pump avatars/follower counts and exposes profit factor plus median winning/losing return in the sample line. Median hold remains null until a validated lot-level hold pipeline exists.
- [x] Add a production-safe Fomo leaderboard identity refresh: resolve the top-50 FomoScan handles through the public profile contract, persist the exact profile/wallet evidence without falsely attaching it to a token, and run one bounded 90-day Dune batch for newly verified wallets. FomoScan's rolling-24h PnL remains the native value; WHOAPED WR, realized PnL, weighted return, profit factor, and median winner/loser return appear only where the wallet metrics exist.
- [x] Add service-only `upsert_fomo_leaderboard_profiles` and `read_fomo_verified_wallet_links` RPCs. The production gateway permits only those two narrow RPC calls rather than exposing the `social_profiles` table. Verification confirms `anon_execute=false`, `authenticated_execute=false`, and `service_role_execute=true`.
- [x] Deploy the refresh via commits `fe313e3` and `360b9f8`. Production verification after the initial run found 20 persisted FomoScan profiles, 15 verified wallet links, and 11 Fomo leaderboard rows with real Dune win-rate/return values; all 50 rows retain Fomo's native 24h PnL. The remaining unresolved handles stay explicitly blank and will be retried by the bounded background refresh rather than guessed.
- [ ] Verify a Fomo-supported Solana token with at least one rendered holder profile and token-unit balance. Record capture/link precision before presenting Fomo-holder coverage as production-complete; Fomo currently renders no holder profile rows for some supported tokens and none at all for tokens it does not list.
