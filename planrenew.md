...
- [x] Build the durable authorized collector: headless Chromium runs inside a bounded Vercel Function, Supabase Cron wakes it every five minutes, a dedicated Fomo session is AES-256-GCM encrypted at rest, refreshes are leased to prevent overlap, and all four leaderboard windows are persisted through the existing first-party contract. One-time dedicated-account connection remains an operational setup step; no personal browser session is copied or logged.
