...
begin
  if jsonb_typeof(identities) <> 'array' or jsonb_array_length(identities) > 100 then
    raise exception 'invalid Fomo identity batch';
  end if;
  for item in select value from jsonb_array_elements(identities)
  loop
    if coalesce(item->>'wallet', '') = '' or coalesce(item->>'platform_profile_id', '') = '' then
      continue;
    end if;
    insert into public.social_profiles (
      ...
      item->>'profile_url', jsonb_strip_nulls(jsonb_build_object('source', item->>'source', 'avatar_url', nullif(item->>'avatar_url', ''))),
    ...
    if not exists (select 1 from public.wallet_links where profile_id = resolved_profile_id and wallet = item->>'wallet' and valid_to is null) then
      ...
      values (resolved_profile_id, 'solana', item->>'wallet', 'fomoscan', 'verified', item->>'profile_url', (item->>'observed_at')::timestamptz);
    end if;

    insert into public.token_social_actors (mint, profile_id, wallet, relationship, confidence, observed_at, updated_at)
    values (identity_mint, resolved_profile_id, item->>'wallet', item->>'relationship', 'verified', (item->>'observed_at')::timestamptz, (item->>'observed_at')::timestamptz)
    on conflict (mint, profile_id, wallet) do update set
      relationship = excluded.relationship, confidence = excluded.confidence, observed_at = excluded.observed_at, updated_at = excluded.updated_at;
    affected := affected + 1;
  end loop;
  return affected;
end;
