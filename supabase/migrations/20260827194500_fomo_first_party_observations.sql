create table if not exists public.fomo_leaderboard_observations (
  period text not null check (period in ('24h', '7d', '30d', 'all')),
  normalized_handle text not null,
  handle text not null,
  display_name text,
  avatar_url text,
  platform_rank integer not null check (platform_rank > 0),
  ...
  primary key (period, normalized_handle, captured_at)
);

create index if not exists fomo_leaderboard_latest_idx
  on public.fomo_leaderboard_observations (period, captured_at desc, platform_rank asc);

create table if not exists public.fomo_identity_evidence (
  id bigint generated always as identity primary key,
  profile_id uuid not null references public.social_profiles(id) on delete cascade,
  wallet text not null,
  mint text references public.tokens(mint) on delete cascade,
  resolution_method text not null check (resolution_method in ('unique_rounded_balance', 'transaction_fingerprint')),
  resolver_version text not null,
  evidence jsonb not null default '{}'::jsonb,
  source_url text not null,
  observed_at timestamptz not null,
  created_at timestamptz not null default timezone('utc', now()),
  unique (profile_id, wallet, mint, resolver_version)
);

create index if not exists fomo_identity_evidence_wallet_idx
  on public.fomo_identity_evidence (wallet, observed_at desc);
create index if not exists fomo_identity_evidence_mint_idx
  on public.fomo_identity_evidence (mint) where mint is not null;

alter table public.fomo_token_holder_observations
  add column if not exists avatar_url text,
  add column if not exists thesis_text text,
  add column if not exists trade_events jsonb not null default '[]'::jsonb,
  add column if not exists resolution_evidence jsonb not null default '{}'::jsonb;

alter table public.fomo_token_holder_observations drop constraint if exists fomo_token_holder_observations_resolution_method_check;
alter table public.fomo_token_holder_observations add constraint fomo_token_holder_observations_resolution_method_check
  check (resolution_method in ('none', 'unique_rounded_balance', 'transaction_fingerprint'));

alter table public.fomo_leaderboard_observations enable row level security;
alter table public.fomo_identity_evidence enable row level security;

revoke all on table public.fomo_leaderboard_observations, public.fomo_identity_evidence from public, anon, authenticated;
revoke all on table public.fomo_token_holder_observations from public, anon, authenticated;
grant select, insert, update on table public.fomo_leaderboard_observations to service_role;
grant select, insert on table public.fomo_identity_evidence to service_role;
grant select, insert, update on table public.fomo_token_holder_observations to service_role;
grant select on table public.social_profiles, public.wallet_links to service_role;
grant usage, select on sequence public.fomo_identity_evidence_id_seq to service_role;
