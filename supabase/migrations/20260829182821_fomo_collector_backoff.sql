...
begin
  ...
  insert into public.fomo_collector_state (id, sealed_session, status, lease_token, lease_until, last_error, next_attempt_at, updated_at)
  values ('primary', new_sealed_session, 'ready', null, null, null, null, timezone('utc', now()))
...
declare
  affected integer;
  message text := left(coalesce(failure_message, 'Unknown collector failure'), 1000);
begin
  update public.fomo_collector_state
  set status = 'error',
      lease_token = null,
      ...
      next_attempt_at = case
        -- Browserbase has rejected the account for quota/billing. Retrying every
        -- five minutes only burns invocations and floods logs; a reconnect or
        -- funded account resets this immediately through set_fomo_collector_session.
        when message ilike '%Browserbase request failed (402)%' then timezone('utc', now()) + interval '6 hours'
...
drop function if exists public.get_fomo_collector_status();

create function public.get_fomo_collector_status()
returns table (
  status text,
...
as $$
  select status, last_attempt_at, last_success_at, last_error,
         consecutive_failures, next_attempt_at, last_counts, updated_at
